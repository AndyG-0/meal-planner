...
def generate_nutritional_info(category: str) -> dict:
    ...
    # Calculate macros based on calories with safe ranges
    protein_max = max(6, int(calories * 0.3 / 4))
    protein = random.randint(5, protein_max)

    carbs_max = max(11, int(calories * 0.5 / 4))
    carbs = random.randint(10, carbs_max)

    fat_max = max(6, int(calories * 0.35 / 9))
    fat = random.randint(5, fat_max)
